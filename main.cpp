...
// объявление флага выхода из ux.cpp
extern bool g_shouldExit;

static std::mt19937 rng((unsigned)time(nullptr));

// прогноз минимакса (для вывода на экран)
static std::string g_minimaxPredict;

// F:\MYPROG\DURAK2>g++ main.cpp -o durak
// -------------------------------------------------------
...
// ------------------------------------------------------------
// PRINT_UX_DIFF — FULLY REWRITTEN FOR UX 2.0
// ------------------------------------------------------------
double eval_state(const State &st);
bool is_endgame(const State &st);

// ------------------------------------------------------------
// CALC EMOTION — 1 (joy), 0 (normal), -1 (sadness)
// ------------------------------------------------------------
static int calc_emotion(double evalVal, bool endgame, const string &mmPred, string &outName)
{
    // терминальные состояния
    if (evalVal >= 8000) { outName = "JOY";    return 1; }
    if (evalVal <= -8000) { outName = "SADNESS"; return -1; }
    if (evalVal == 5000) { outName = "NORMAL";   return 0; }

    // эндшпиль — по прогнозу минимакса
    if (endgame)
    {
        if (mmPred == "BOT WIN")   { outName = "JOY";    return 1; }
        if (mmPred == "NOT ASSUMED") { outName = "NORMAL";   return 0; }
        if (mmPred == "PLR WIN")   { outName = "SADNESS"; return -1; }
    }

    // до эндшпиля — по eval
    if (evalVal <= -50) { outName = "SADNESS"; return -1; }
    if (evalVal >= 50)  { outName = "JOY";    return 1; }
    outName = "NORMAL"; return 0;
}
...
void print_ux_diff(const State &old, const State &st)
{
    ...
    // ---------------------------------------------------------
    // 0. STARTING DEAL — ПОЧЕРЕДНАЯ РАЗДАЧА
    // ---------------------------------------------------------
    if (old.hand_plr.empty() && old.hand_bot.empty() &&
        st.hand_plr.size() == 6 && st.hand_bot.size() == 6)
    {
        // Сбрасываем эмоцию на нейтральную перед первой раздачей
        ux_cmd("UPDATE_EMOTION", std::vector<std::string>{"NORMAL"});

        // Сбрасываем флаг подсказки для новой игры
        hintShown = false;

        // Сначала показываем полную колоду (36 карт)
        ux_cmd("UPDATE_DECK_SIZE",
               {std::to_string(36)});

        // Проигрываем звук колоды и ждём завершения (200 мс)
        ux_cmd("PLAY_KOLODA_SOUND", {});
        ux_cmd("WAIT", {"200"});
        ...
        ux_cmd("SET_TRUMP",
               {card_to_string(st.trump_card)});

        ux_cmd("UPDATE_DECK_SIZE",
               {std::to_string(st.deck.size())});

        ux_cmd("SET_ACTION_BUTTON", {"NONE"});

        // если первый ход игрока — показываем подсказку
        if (st.attacker == PLR)
        {
            ux_cmd("SHOW_HINT", {});
        }
    ...
    // ---------------------------------------------------------
    // 2. TABLE_TO_HAND (defender takes)
    // ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // 4. UPDATE_DECK_SIZE (normal)
    // ---------------------------------------------------------
    if (old.deck.size() != st.deck.size() &&
        st.deck.size() >= 1)
    {
        ux_cmd("UPDATE_DECK_SIZE",
               std::vector<std::string>{std::to_string(st.deck.size())});
    }

    // ---------------------------------------------------------
    // 5. Detect removed cards from deck
    // ---------------------------------------------------------
    ...
    auto detect_draw_set = [&](Side s)
    {
        ...
        bool prev_trump_added = false;

        for (auto &c : added)
        {
            if (removed_prev_trump && c == prev_trump)
            {
                ux_cmd("DEAL_PREVOISE_TRUMP",
                       std::vector<std::string>{
                           (s == PLR ? "PLR" : "BOT"),
                           card_to_string(c)});
                prev_trump_added = true;
                continue;
            }

            if (removed_trump && c == trump)
            {
                // Ждём завершения анимации предпоследней карты
                if (prev_trump_added)
                    ux_cmd("WAIT", {"120"});

                ux_cmd("DEAL_LAST_TRUMP",
                       std::vector<std::string>{
                           (s == PLR ? "PLR" : "BOT"),
                           card_to_string(c)});
                continue;
    ...
    // ---------------------------------------------------------
    // 7. UPDATE_DECK_SIZE 0
    // ---------------------------------------------------------
    if (st.deck.empty() && !old.deck.empty())
    {
        ux_cmd("UPDATE_DECK_SIZE", std::vector<std::string>{"0"});
    }
    ...
    if (!old_is_go && new_is_go)
    {
        if (ph_new == PH_GAMEOVER_PLR_WIN)
            ux_cmd("GAMEOVER", std::vector<std::string>{"PLR_WIN"});
        else if (ph_new == PH_GAMEOVER_BOT_WIN)
            ux_cmd("GAMEOVER", std::vector<std::string>{"BOT_WIN"});
        else
            ux_cmd("GAMEOVER", std::vector<std::string>{"DRAW"});

        // эмоция для терминального состояния
        ...
        ux_cmd("SET_ACTION_BUTTON", std::vector<std::string>{"CONTINUE"});
        return;
    }
    ...
    // ход игрока — показываем подсказку в первый раз (дублирующая проверка)
    if (!hintShown && st.actor == PLR && old.table.empty() && st.table.empty())
    {
        ux_cmd("SHOW_HINT", {});
        hintShown = true;
    }
    ...
    if (ph == PH_DEFENSE_CONTINUE)
        ux_cmd("SET_ACTION_BUTTON", std::vector<std::string>{"TAKE"});
    else if (ph == PH_ATTACK_CONTINUE)
        ux_cmd("SET_ACTION_BUTTON", std::vector<std::string>{"BEAT"});
    else if (ph == PH_EXTRA_ATTACK)
    {
        if (st.actor == st.attacker)
            ux_cmd("SET_ACTION_BUTTON", std::vector<std::string>{"GIVE"});
        else
            ux_cmd("SET_ACTION_BUTTON", std::vector<std::string>{"TAKE"});
    }
    else
...
State init_next_game_state(Side attacker)
{
    State st;
    ...
    while (true)
    {
        ...
        st.hand_plr.assign(full.begin(), full.begin() + 6);
        st.hand_bot.assign(full.begin() + 6, full.begin() + 12);
...
Move midgame_choose(const State &st)
{
    ...
    if (X == A)
    {
        ...
        auto attack_filter = [&](const Move &m) -> bool
        {
            if (m.is_pass)
                return false;
            //----------------не подбрасывать козырь когда противник берёт
            Phase ph = what_ph(st);
...
Move endgame_choose(State &st)
{
    ...
    if (bestVal >= 7000)
    {
        ...
        g_minimaxPredict = "BOT WIN";
    }
    else if (bestVal <= -7000)
    {
        cout << "===== Assumed PLR win  =====\n";
        g_minimaxPredict = "PLR WIN";
    }
    else
    {
        cout << "=== NOT Assumed =========\n";
        g_minimaxPredict = "NOT ASSUMED";
    }
...
bool parse_card_safe(const string &s, Card &out)
{
    if (s.size() < 2)
        return false;
    ...
    int r = 0;
    if (rankStr == "J" || rankStr == "j")
        r = 11;
    else if (rankStr == "Q" || rankStr == "q")
        r = 12;
    else if (rankStr == "K" || rankStr == "k")
        r = 13;
    else if (rankStr == "A" || rankStr == "a")
        r = 14;
    else
    {
        try
...
/*
int get_player_move_index(const State &st, const vector<Move> &moves)
{
    while (true)
    {
        cout << "Valid moves: ";
        ...
        cout << "Enter card or p for PASS, or q to quit: ";
        ...
        if (line == "p" || line == "P")
        {
            ...
            cout << "PASS is not allowed now.\n";
            continue;
        }

        // попытка распарсить карту
        Card input;
        bool ok = parse_card_safe(line, input); // мы напишем эту функцию ниже
        if (!ok)
        {
            cout << "Invalid card format.\n";
            continue;
        }

        // ищем карту среди валидных ходов
        for (int i = 0; i < (int)moves.size(); i++)
        {
            if (!moves[i].is_pass && same_card(moves[i].card, input))
                return i;
        }

        cout << "This card is not a valid move.\n";
    }
} */
int get_player_move_index(const State &st, const std::vector<Move> &moves)
{
    // 1) преобразуем moves → строки
    std::vector<std::string> moveStrings;
    moveStrings.reserve(moves.size());

    for (auto &m : moves)
    {
        if (m.is_pass)
            moveStrings.push_back("p");
        else
            moveStrings.push_back(card_to_string(m.card)); // функция движка
    }

    // 2) запускаем UX-режим
    ux_start_wait_player_move(moveStrings);
    ...
    // 4) возвращаем выбранный индекс
    return ux_get_chosen_move();
}
// -------------------------
// MAIN LOOP
// -------------------------
...
int main()
{
    ...
    // Выводим для отладки
    std::cout << "Desktop resolution: " << desktop.width << "x" << desktop.height << "\n";

    // Минимальное поддерживаемое разрешение: 1920×1080 (Full HD)
    const unsigned int MIN_WIDTH = 1920;
    const unsigned int MIN_HEIGHT = 1080;

    if (desktop.width < MIN_WIDTH || desktop.height < MIN_HEIGHT)
    {
        // Создаём окно для показа ошибки
        sf::RenderWindow errorWindow(sf::VideoMode(800, 200), "Ошибка", sf::Style::Close | sf::Style::Titlebar);
        ...
        errorText.setFillColor(sf::Color(255, 165, 0)); // оранжевый
        errorText.setOutlineColor(sf::Color::Black);
        errorText.setOutlineThickness(2.f);
        errorText.setString(L"Ваше разрешение экрана не поддерживает\nминимальное разрешение 1920x1080 (Full HD)");

        // Центрируем текст
        auto bounds = errorText.getLocalBounds();
        errorText.setOrigin(bounds.width / 2.f, bounds.height / 2.f);
        errorText.setPosition(400.f, 100.f);

        // Показываем 5 секунд с fade-out
        sf::Clock clock;
        float fadeStart = 4.f;    // начинаем затухать на 4-й секунде
        float fadeDuration = 1.f; // длительность затухания 1 сек

        while (errorWindow.isOpen())
        {
            sf::Event e;
            while (errorWindow.pollEvent(e))
            {
            ...
            // Fade-out после 4 секунд
            if (elapsed >= fadeStart)
            {
                float alpha = 255.f * (1.f - (elapsed - fadeStart) / fadeDuration);
                if (alpha < 0.f)
                    alpha = 0.f;
                errorText.setFillColor(sf::Color(255, 165, 0, (sf::Uint8)alpha));
            }
