...
// F:\MYPROG\DURAK2>g++ main_old_3mc.cpp -o durak_txt
// -------------------------------------------------------
...
// -------------------------
// MONTE CARLO ROLLOUTS — MIDGAME V2
// -------------------------

// Одна симуляция до конца игры
// Одна симуляция до конца игры
// BOT — рандом, PLR — midgame_choose (жадный)
double run_simulation(State st, int max_depth = 200)
{
    int depth = 0;
    ...
    while (depth < max_depth)
    {
        ...
        if (ph == PH_GAMEOVER_BOT_WIN)
            return 1.0;
        if (ph == PH_GAMEOVER_PLR_WIN)
            return 0.0;
        if (ph == PH_GAMEOVER_DRAW)
            return 0.5;
        ...
        auto res = validator(st);
        if (res.error || res.moves.empty())
        {
            return (st.actor == BOT ? 0.0 : 1.0);
        }
        ...
        if (st.actor == BOT)
        {
            // BOT — рандомизированный выбор (исследуем варианты)
            vector<Move> active_moves;
            for (auto &mv : moves)
            {
                if (!mv.is_pass)
                    active_moves.push_back(mv);
            }

            if (!active_moves.empty() && depth < 50)
            {
                m = active_moves[rng() % active_moves.size()];
            }
            else
            {
                m = moves[rng() % moves.size()];
            }
        }
        else
        {
            // PLR — играет через midgame_choose (жадный алгоритм)
            m = midgame_choose(st);
        }
    ...
    return (eval_state(st) > 0 ? 0.5 : 0.5);
}
...
Move midgame_choose2(State &st)
{
    auto start_time = chrono::steady_clock::now();
    const double TIME_LIMIT = 2.0;    // 2 секунды
    const int MIN_SIMULATIONS = 20;   // Минимум симуляций на ход
    const int MAX_SIMULATIONS = 1000; // Максимум если есть время

    cout << "\n=== MC ACTIVATED (deck=" << st.deck.size() << ") ===\n";
    ...
    // === ВКЛЮЧАЕМ ВСЕ ХОДЫ, ВКЛЮЧАЯ PASS ===
    vector<Move> candidate_moves = moves;
    ...
    {
        scores.push_back({m, 0.5, 0, 0}); // Старт с 0.5 (нейтрально)
    }

    // === СИМУЛЯЦИИ: продолжаем пока есть время И не достигли минимума ===
    int total_sims = 0;
    for (int sim = 0; sim < MAX_SIMULATIONS; sim++)
    {
        ...
        // Проверка: все ли ходы достигли минимума симуляций
        bool all_at_min = true;
        for (int i = 0; i < (int)scores.size(); i++)
        {
            if (scores[i].sims < MIN_SIMULATIONS)
            {
                all_at_min = false;
                break;
            }
        }
        if (all_at_min && elapsed > TIME_LIMIT * 0.5)
            break; // После минимума можно остановиться раньше

        for (int i = 0; i < (int)scores.size(); i++)
        {
            if (scores[i].sims >= MAX_SIMULATIONS)
                continue;

            State next = apply_move(st, scores[i].move);
            double result = run_simulation(next);

            scores[i].wins += (int)(result * 2);
            scores[i].sims++;
            scores[i].win_rate = (double)scores[i].wins / (2.0 * scores[i].sims);
            total_sims++;
        }
    }
    ...
    // Выбор лучшего хода
    int best_idx = 0;
    double best_rate = scores[0].win_rate;

    for (int i = 1; i < (int)scores.size(); i++)
    {
        // Бонус за количество симуляций (более надёжная статистика)
        double adjusted = scores[i].win_rate + (scores[i].sims * 0.001);
        double best_adjusted = best_rate + (scores[best_idx].sims * 0.001);
        if (adjusted > best_adjusted)
        {
            best_rate = scores[i].win_rate;
            best_idx = i;
        }
    }

    cout << "[MC] Total sims: " << total_sims << " | Time: "
         << chrono::duration<double>(chrono::steady_clock::now() - start_time).count() << "s\n";
    cout << "[MC] Win rates: ";
    for (auto &s : scores)
    {
        if (s.move.is_pass)
            cout << "PASS:" << s.win_rate << "(" << s.sims << ") ";
        else
            cout << rank_to_str(s.move.card.rank) << suit_to_str(s.move.card.suit)
                 << ":" << s.win_rate << "(" << s.sims << ") ";
    }
    cout << "\n";

    return scores[best_idx].move;
}
Move bot_choose(State &st)
{
    ...
    {
        // Переключатель MC/эвристика по размеру колоды
        if (st.deck.size() <= 16)
            return midgame_choose2(st); // MC симуляции
        else
            return midgame_choose(st); // Эвристики
    }
}
