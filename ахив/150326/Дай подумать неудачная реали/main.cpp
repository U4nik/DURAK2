...
// флаг показа надписи "Дай подумать" (1 раз за игру)
static bool g_thinkingShown = false;
...
void print_ux_diff(const State &old, const State &st)
{
        ...
        st.hand_plr.size() == 6 && st.hand_bot.size() == 6)
    {
        ...
        // Сбрасываем флаг "Дай подумать" для новой игры
        g_thinkingShown = false;
    ...
    // бот только что походил → скрываем надпись "Дай подумать"
    if (old.actor == BOT && st.actor == PLR)
    {
        ux_cmd("HIDE_THINKING", {});
    }
...
Move bot_choose(State &st)
{
    if (is_endgame(st))
    {
        // показываем надпись "Дай подумать" (1 раз за игру)
        if (!g_thinkingShown)
        {
            ux_cmd("SHOW_THINKING", {});
            g_thinkingShown = true;
        }
...
int main()
{
    ...
    while (window.isOpen())
    {
            ...
            ph == PH_GAMEOVER_DRAW)
        {
            ...
            // сброс флага "Дай подумать" для новой игры
            g_thinkingShown = false;
