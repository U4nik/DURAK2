...
// текст "Дай подумать" (бот думает)
static sf::Text g_thinkingText;
static bool g_thinkingVisible = false; // видима ли надпись
static bool g_thinkingFading = false;  // флаг затухания
static float g_thinkingAlpha = 255.f;  // альфа для затухания
...
// ------------------------------------------------------------
// "ДАЙ ПОДУМАТЬ" - БОТ ДУМАЕТ
// ------------------------------------------------------------
static void show_thinking_text()
{
    g_thinkingText.setString(L"Дай подумать...");

    // центрируем
    auto b = g_thinkingText.getLocalBounds();
    g_thinkingText.setOrigin(b.width / 2.f, b.height / 2.f);
    g_thinkingText.setPosition(1920.f / 2.f, 1080.f / 2.f);

    g_thinkingAlpha = 255.f;
    g_thinkingVisible = true;
    g_thinkingFading = false;
}

static void hide_thinking_text()
{
    if (g_thinkingVisible && g_thinkingAlpha > 0.f)
        g_thinkingFading = true;
}
...
void reset_all_visuals()
{
    ...
    // сброс "Дай подумать"
    g_thinkingVisible = false;
    g_thinkingFading = false;
    g_thinkingAlpha = 255.f;
    g_thinkingText.setString(L"");
}
...
void ux_init(sf::RenderWindow *win)
{
    ...
    // текст "Дай подумать" (бот думает)
    g_thinkingText.setFont(g_font);
    g_thinkingText.setCharacterSize(64);
    g_thinkingText.setFillColor(sf::Color(255, 165, 0)); // оранжевый
    g_thinkingText.setString(L"Дай подумать...");
    g_thinkingText.setPosition(1920.f / 2.f, 1080.f / 2.f);
...
void ux_run_command(const UxCommand &cmd)
{
    ...
    else if (cmd.name == "SHOW_THINKING")
    {
        show_thinking_text();
    }

    else if (cmd.name == "HIDE_THINKING")
    {
        hide_thinking_text();
    }
...
static void ux_draw_frame()
{
    ...
    // "Дай подумать" (если видимо)
    if (g_thinkingVisible || g_thinkingFading)
        g_window->draw(g_thinkingText);
...
void ux_process_frame()
{
    ...
    // анимация "Дай подумать"
    if (g_thinkingFading)
    {
        g_thinkingAlpha -= 300.f * dt; // 300 в секунду
        if (g_thinkingAlpha < 0.f)
            g_thinkingAlpha = 0.f;
        g_thinkingText.setFillColor(sf::Color(255, 165, 0, (sf::Uint8)g_thinkingAlpha));
