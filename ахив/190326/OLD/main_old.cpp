...
void print_ux_diff(const State &old, const State &st)
{
    ...
    // ---------------------------------------------------------
    // 0. STARTING DEAL (init_first_game_state)
    // ---------------------------------------------------------
    ...
    // 1. LAST TRUMP (козырь)
    // ---------------------------------------------------------
    if (old.deck.size() == 1 && st.deck.size() == 0)
    {
        cout << "[UX] DEAL_LAST_TRUMP ";
        print_card(st.trump_card);
        cout << "\n";

        cout << "[UX] REMOVE_TRUMP\n";
    }

    // ---------------------------------------------------------
    // 2. PLAY_ATTACK / PLAY_DEFENSE
    // ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // 3. TABLE_TO_HAND (защитник взял стол)
    // ---------------------------------------------------------
    if (!old.table.empty() && st.table.empty() &&
        old.discard.size() == st.discard.size()) // защитник взял
    {
    ...
    // ---------------------------------------------------------
    if (!old.table.empty() && st.table.empty() &&
        old.discard.size() < st.discard.size()) // карты ушли в отбой
    {
        cout << "[UX] CLEAR_TABLE ";
        for (auto &p : old.table)
        {
    ...
    // ---------------------------------------------------------
    // 5. DRAW UP TO 6 (добор через множества)
    // ---------------------------------------------------------
    ...
    // Атакующий добирает ВСЕГДА
    detect_draw_set(old.attacker);

    // Защитник добирает ТОЛЬКО если НЕ брал стол
